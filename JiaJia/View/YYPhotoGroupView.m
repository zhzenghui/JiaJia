...
@property (nonatomic, strong) UIView *saveAllView;
...
- (void)saveAll {
    NSArray *cells = (NSArray<YYPhotoGroupCell *> *)self.cells;
    NSMutableArray *array = [[NSMutableArray alloc] init];;

    NSLog(@"所有图片：%i", (int)cells.count);
    __block int successCount = 0;
    __block int faildCount = 0;
    for (YYPhotoGroupCell *cell in cells) {
        NSLog(@"%@", cell.item.largeImageURL);
//        UIImageView *iv = [[UIImageView alloc] init];        
//        @weakify(self);
//        [iv setImageWithURL:cell.item.largeImageURL placeholder:nil options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
//            @strongify(self);
//            if (!self) return;
//            CGFloat progress = receivedSize / (float)expectedSize;
//            progress = progress < 0.01 ? 0.01 : progress > 1 ? 1 : progress;
...
//            else {
//                faildCount++;
//                NSLog(@"个数：%i", faildCount);
//            }
//
//        }];
    }
//    [[PhotoLibraryHelper shareInstance] saveImages:array];
}
